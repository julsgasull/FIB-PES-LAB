  ...
  CommandLineRunner initUserDatabase(UserService service) {
    ...
    CommandLineRunner initDeviceDatabase(DeviceService service) {
        return args -> {
            ...
            // Location "bar"
            logger.info("Preloading " + service.saveDevice(new Device(null, new Location((float)41.447379, (float)2.226842, (float)100, (float)0), new User())));
            // Location "china"
            logger.info("Preloading " + service.saveDevice(new Device(null, new Location((float)21.160510, (float)-86.842466, (float)100, (float)0), new User())));
            // Location "otrositio"
            logger.info("Preloading " + service.saveDevice(new Device(null, new Location((float)41.402899, (float)2.121561, (float)100, (float)0), new User())));
        };
    }
}